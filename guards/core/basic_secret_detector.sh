...
hook_basic_secret_detector() {
  ...
  if echo "$CMD_SHELL" | grep -qiP '(curl|wget|fetch|nc)\s.*(webhook\.site|requestbin\.|pipedream\.|hookbin\.|burpcollaborator\.|interact\.sh|canarytokens\.)'; then
  ...
  if echo "$CMD_SHELL" | grep -qiP '(curl|wget)\s.*(-d|--data|--data-raw|--data-binary)\s.*\$\{?(API_KEY|SECRET|TOKEN|PASSWORD|CREDENTIALS|AWS_SECRET)'; then
    deny "SECRET-GUARD: Sending secret variables to external endpoint."
    return
  fi

  # Block base64-encoding of secret variables
  if echo "$CMD_SHELL" | grep -qiP 'echo\s.*\$\{?(API_KEY|SECRET|TOKEN|PASSWORD|AWS_SECRET).*\|\s*base64'; then
